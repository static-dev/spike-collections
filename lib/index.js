...
class SpikeCollections {
  ...
  apply(compiler) {
    ...
    compiler.plugin('before-loader-process', (compilation, options) => {
      this.handlePagination(compiler)
      // match template to its config
      const conf = filter(this.collections, v => {
      ...
      // if it is, add a page per pagination output with that page's locals
      options.multi = []
      map(this._pagination[key], (locals, name) => {
        options.multi.push({ name, locals })
      })
  ...
  handlePagination(compiler) {
    ...
    map(pages, (postData, k, i) => {
      ...
      postData.map((p, i) => {
        ...
        this._pagination[k][p.path] = locals
      })
    })
  }

...
class SpikeCollections {
  ...
  addCollectionFiles(compiler, compilation, done) {
    ...
    const fileAdd = this.util.addFilesAsWebpackEntries(
      compilation,
      this.allFiles
    )
      ...
      .map(this.collections, (v, k) => {
        ...
        return keys.map(this._pagination, (pages, _) => {
          return keys.map(pages, (opts, p) => {
            const reshapeOpts = loader.parseOptions.call(
              opts.loaderContext,
              this.util.getSpikeOptions().reshape,
              { locals: opts.locals }
            )
            // add locals to reshape opts in case of early eval code
            Object.assign(reshapeOpts, { locals: opts.locals })
            return reshape(reshapeOpts).process(tpl).then(res => {

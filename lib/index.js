...
module.exports = class SpikeCollections {
  ...
  addCollectionFiles (compiler, compilation, done) {
    ...
    const postsWithFrontmatter = map(this.files, (v, k) => {
      return v.sort(this.order).map((p) => {
        ...
        // run the permalink function and get those locals if it exists
        // save them to `this`, bc they are accessed by the locals fn later
        const permalinkFn = this.collections[k].permalink
        const permalinkLocals = permalinkFn ? permalinkFn(path.basename(p)) : {}
        const relativePath = this.util.getOutputPath(p).relative
        this._permalinkLocals[relativePath] = permalinkLocals

        // merge all the locals, add the path, and return
        return Object.assign(fm, permalinkLocals, { _path: relativePath })
      })
    })
  ...
  locals (ctx, locals = {}) {
    if (ctx.options.__frontmatter) {
      ...
      const permalinkLocals = this._permalinkLocals[f.relative]
